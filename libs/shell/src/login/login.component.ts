...
import { AuthService, BASE_URL, Credentials, LocalizationService } from '@office/core';
...
@Component({
    selector: 'login',
    imports: [
        CommonModule, FormsModule, CheckboxModule, InputTextModule, 
        RouterModule, IconFieldModule, InputIconModule, ButtonModule, FluidModule
    ],
...
export class LoginComponent implements OnInit, OnDestroy {
        ...
        this.subscription = this.authService.authStatus$
            ...
            .subscribe((status) => {
                if (!this.initialize && !status) {
                    firstValueFrom(this.httpClient.get<Credentials>(`${this.baseUrl}api/account/login`)).then((result) => {
                        this.model = result;
    ...
    submit() {
        this.authService.login(this.model).subscribe({
            ...
            complete: () => {
                if (this.model.captchaEnabled) this.reCaptcha();
            }
        });
    }
}
